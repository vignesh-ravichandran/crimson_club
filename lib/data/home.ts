/**
 * Server-only home data. Used by API route and Home/Review pages so we don't rely on fetch() to own API (avoids subrequest issues on Cloudflare).
 */
...
export async function getHomeData(user: SessionUser): Promise<HomeResponse> {
  const db = getDb();
